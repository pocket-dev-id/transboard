...
const {
  configureParentHttp,
  parentHttpRequest,
} = require('./main-modules/parent-http');
const {
...
} = require('./main-modules/transfer-status');
const {
  configureUpdater,
  registerUpdaterIpc,
} = require('./main-modules/updater');
const {
  MASKED_SECRET_VALUE,
...
configureUpdater({
  ...
  EXPECTED_UPDATE_PUBLISHER_THUMBPRINT,
});
...
handleTrusted('db-request', async (event, { url, options }) => {
  ...
  try {
    if (!isAllowedLocalDbRequestUrl(url, ALLOWED_TABLES)) {
      return { success: false, message: 'Not Found' };
    }
...
handleTrusted('webrtc-request', async (event, { url, options }) => {
  if (!isAllowedWebrtcRequestUrl(url)) {
    return { success: false, message: 'Not Found' };
  }
  const method = (options.method || 'GET').toUpperCase();
...
registerUpdaterIpc(handleTrusted);
...
function startParentServer() {
  ...
  // 親機モードの LAN 待受。病棟内の子機から接続するため 0.0.0.0 にする。
  // 子機モードでは startParentServer() 自体を呼ばない。127.0.0.1 にはしない。
