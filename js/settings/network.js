  ...
  async _renderNetworkSettings(body) {
    ...
    const modeGuideItems = isClientMode
      ...
      : [
          ...
          ['fa-file-import', '親機で実行する処理', '取り込み、スケジュールは「取り込み設定」タブ、更新配信・バックアップは「システム保守」タブで管理します。', '__tab:maintenance'],
    ...
    body.querySelectorAll('.settings-guide-card').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        // 他タブへ移設した機能への案内カードは、スクロールではなくタブ切り替えを行う
        if (targetId?.startsWith('__tab:')) {
          Settings._activeTab = targetId.slice('__tab:'.length);
          Settings.render();
          return;
        }
