...
|---|---|---|
| `id` | string | 主キー |
| `name` | string | 検査室名（例: `CT室`） |
| `code` | string | コード（例: `CT`） |
| `floor` | string \| null | 階 |
| `phone` | string \| null | 内線番号 |
| `icon` | string | 表示アイコン（Font Awesomeクラス） |
| `note` | string \| null | 備考 |
| `is_active` | boolean | 有効/無効 |
| `exam_type_ids` | string[] | この検査室で実施できる検査種別ID |

`exam_type_ids` は出棟登録フォームの絞り込みに使う。**フィールド自体が無い場合と
空配列の場合はどちらも「すべての検査種別に対応」**として扱うため、この
フィールドを持たない既存データはそのまま従来どおり全種別を選択できる。
行き先検査室を選ぶと、対応しない検査種別のカードは非表示になる。
...
### `chat_messages` — 端末間チャット・アナウンス送信履歴

病棟・検査室の論理ID同士（1対1）でやりとりするテキストチャット(`kind:'chat'`)と、
アナウンス送信履歴(`kind:'announce'`)を同じテーブルに時系列で保持する追記専用テーブルです。
表示先はkindによって異なり、`chat`は通話パネル内の会話画面が、`announce`は宛先(`to_id`)側の
通知履歴パネルが、それぞれこの1テーブルから該当分だけを描画します（後述）。
...
**表示先:** `kind:'chat'`はチャットのタイムライン（`API.getChatMessages`、`conversation_key`で絞り込み）
にのみ表示され、`kind:'announce'`はチャット側には表示せず、`to_id`が自分（病棟/検査室）と一致する分だけを
...
| ファイル | 内容 | 備考 |
|---|---|---|
| `terminal_role.json` | `{shareMode, parentIp, terminalRole, wardId, deviceName, preventSleep, alwaysOnTop, updatedAt}` | 平文。起動時にこのファイルを正としてDBの`share_mode`/`parent_ip`を修復する。`wardId`/`deviceName`/`preventSleep`/`alwaysOnTop`は配布時に投入される既定値で、利用者がまだ選択/変更していない場合のみ画面側に採用される |
