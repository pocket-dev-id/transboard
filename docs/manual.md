...
#### サイレントインストーラへの引数指定（1台ずつ持ち歩いて導入する場合）

`provisioning.json`を事前に配置する代わりに、インストーラ（`.exe`）を
コマンドライン引数付きでサイレント実行（`/S`）することでも同じ内容を
投入できます。台数分の`provisioning.json`を個別に用意しなくても、
USBメモリ等でインストーラを持ち歩いて各端末で1行実行するだけで済みます。
per-user版（既定の`npm run dist`）・per-machine版（`npm run dist:per-machine`）
の**どちらのインストーラにも対応**しています。

**方法1: 事前に用意したprovisioning.jsonを指定する（推奨・より安全）**

```
TransBoard-Setup-1.2.83.exe /S /ACCOUNT=commonuser /PROVISIONING=D:\provisioning-ward1.json
```

APIトークンをこのインストーラ自身のコマンドラインには載せないため、
[方法2]より安全です。指定したファイルはそのまま配置先へコピーされます
（内容の妥当性はTransBoard起動時に検証されます）。
...
| 引数 | 説明 |
|---|---|
| `/ACCOUNT=<共通アカウント名>` | 投入先ユーザーを明示する。**マシン単位インストーラをPsExecの既定（資格情報省略）で実行する場合は必須**。省略すると、インストーラを実行しているアカウント（PsExecの既定ではSYSTEM）のプロファイルに投入されてしまい、実際に使う共通アカウントからは見えない |
...
- 上記いずれの引数も指定しない場合、通常のインストールと挙動は変わりません（`provisioning.json`は作成されません）。
