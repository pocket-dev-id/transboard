...
const CONFIG = {
  ...
  getNearlyDoneMinutes() {
    try { return AppState.getSettingInt('nearly_done_minutes', 10); } catch { return 10; }
  },

  getSoonThresholdMinutes() {
    try { return AppState.getSettingInt('soon_threshold_min', 15); } catch { return this.SOON_THRESHOLD_MIN || 15; }
  },

  getTimingLabel(status) {
    if (status === 'NEARLY_DONE') {
      try {
        const custom = AppState.getSettingJSON('status_custom_labels', {});
        if (custom?.NEARLY_DONE) return custom.NEARLY_DONE;
      } catch {}
      return `あと${this.getNearlyDoneMinutes()}分`;
    }
    if (status === 'SOON') return `お迎え目安まであと${this.getSoonThresholdMinutes()}分`;
  ...
  getAllowedActions(status, scope = 'ward') {
    ...
    const hidden = this.getHiddenStatuses();
    const actions = [...(source[status] || [])];
    if (typeof TRANSFER_RULES === 'undefined') return actions;
    const allowed = TRANSFER_RULES.allowedTargets(status, hidden, scope);
    if (!hidden.includes('ARRIVED')) return actions.filter(action => allowed.includes(action.toStatus));
    const expanded = [];
    for (const action of actions) {
      if (action.toStatus === 'ARRIVED') expanded.push(...(source.ARRIVED || []).filter(item => allowed.includes(item.toStatus)));
