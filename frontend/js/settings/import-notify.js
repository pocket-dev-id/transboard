...
Object.assign(Settings, {
  ...
  _renderNotificationSettings(body) {
    ...
    items.find(item => item.key === 'NEARLY_DONE').label = `あと${CONFIG.getNearlyDoneMinutes()}分（検査終了見込み${CONFIG.getNearlyDoneMinutes()}分前）`;
    items.find(item => item.key === 'SOON').label = `お迎え目安${CONFIG.getSoonThresholdMinutes()}分前（登録済みお迎え時刻の${CONFIG.getSoonThresholdMinutes()}分前）`;
                ...
                <tr>
                  ...
                  <td>
                    <div style="font-weight:700;font-size:13px;color:#2d3748;">
                      ${CONFIG.getTimingLabel(item.key)}
                    </div>
