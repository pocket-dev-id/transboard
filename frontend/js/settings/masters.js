...
Object.assign(Settings, {
  ...
  // ──────────────────────────────────
  _parseCSV(text) {
    return CSVService.parse(text);
  },

  _generateCSV(headers, rows) {
    return CSVService.generate(headers, rows);
  },
              ...
              for (let i = 1; i < rows.length; i++) {
                ...
                headers.forEach(h => {
                  const idx = csvHeaders.indexOf(h);
                   ...
                   // Convert fields to expected types through the shared CSV service.
                   try {
                     const type = ['map_col', 'map_row', 'sort_order', 'standard_duration_min'].includes(h)
                       ? 'integer'
                       : h === 'is_active' ? 'boolean' : 'default';
                     val = CSVService.coerce(rawValue, type);
                   } catch {
