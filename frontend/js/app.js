...
const PasscodeModal = {
  ...
  async submit() {
    ...
    if (ok) {
      ...
      window.maintenanceToken = verification.maintenanceToken || null;
      this.unlock();
...
const App = {
  ...
  async resetActiveEvents() {
    ...
    if (window.electronAPI) {
      const result = await window.electronAPI.resetDatabase(window.maintenanceToken || '');
      if (result?.success === false) throw new Error(result.message || 'リセット権限がありません');
    } else {
  ...
  _checkNotifications() {
    ...
    (AppState.todayEvents || []).forEach(e => {
      ...
      // NEARLY_DONE通知（設定した見込み時間を表示）
      if (e.current_status === 'NEARLY_DONE' && !this._prevNotified.has(`nearly-${e.id}`)) {
        const bed = AppState.getBedById(e.bed_id);
        const cfg = soundSettings['NEARLY_DONE'];
        if (cfg?.toast !== false) {
          this._showStatusNotificationToast(`⏰ ${bed ? bed.bed_number + '号床' : ''} ${CONFIG.getTimingLabel('NEARLY_DONE')}です`, 'warning', 5000, e);
        }
        this._prevNotified.add(`nearly-${e.id}`);
      }

      // 迎え目安の設定時間前通知（時刻経過による特別トリガー）
      if (e.estimated_pickup_at && !this._prevNotified.has(`soon-${e.id}`)) {
        const remaining = e.estimated_pickup_at - now;
        const soonThresholdMin = CONFIG.getSoonThresholdMinutes();
        if (remaining > 0 && remaining <= soonThresholdMin * 60 * 1000) {
          const bed = AppState.getBedById(e.bed_id);
          const cfg = soundSettings['SOON'];
          if (cfg?.toast !== false) {
            UI.toast(`⚠️ ${bed ? bed.bed_number + '号床' : ''} ${CONFIG.getTimingLabel('SOON')}`, 'warning', 5000);
          }
